...
## also make explicit the missing values are ""
## make sure the sourcefile is set first
## sourcefile = "~/Documents/R/MAP Predictive Analytics/RawData/2009 09 Fall.txt"
...
lang <- data %>% filter(MeasurementScaleName == "Language Usage") %>% 
  rename(langTestName = TestName, langRIT = TestRITScore, langPercentile = TestPercentile, langStdErr = TestStdErr, GramUs = GoalRITScore1, SpPunctCap = GoalRITScore2, ParaSent = GoalRITScore3, GenPurpAud = GoalRITScore4) %>% 
...
read <- data %>% filter(MeasurementScaleName == "Reading") %>% 
  rename(readTestName = TestName, readRIT = TestRITScore, readPercentile = TestPercentile, readStdErr = TestStdErr, ReadProc = GoalRITScore1, LitTex = GoalRITScore2, NFictTex = GoalRITScore3) %>% 
